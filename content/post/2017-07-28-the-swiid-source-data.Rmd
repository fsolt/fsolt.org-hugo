...
_Friday, 27 October 2017: Updated with information on the source data for SWIID Version 6.1_
...
The resulting dataset comprises `r nrow(swiid_source)`(!) Gini coefficients from `r swiid_source %>% count(country, year) %>% nrow()` country-years in `r swiid_source %>% count(country) %>% nrow()` countries or territories, making the coverage of the SWIID source data broader than that of any other income inequality dataset.  This isn't surprising given that, with the exceptions of the [WIID](https://www.wider.unu.edu/database/world-income-inequality-database-wiid34) (which, since it provides no original data, isn't drawn on at all anymore) and the [All the Ginis](https://www.gc.cuny.edu/Page-Elements/Academics-Research-Centers-Initiatives/Centers-and-Institutes/Stone-Center-on-Socio-Economic-Inequality/Core-Faculty,-Team,-and-Affiliated-LIS-Scholars/Branko-Milanovic/Datasets) database (which provides little original data, and so isn't drawn on much), the SWIID source data incorporates all of the data in these other datasets.  
